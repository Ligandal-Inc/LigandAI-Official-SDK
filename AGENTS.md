...
| **super-elite (affinity)** | structural gate AND predicted Kd < 100 nM (DeltaForge) | Synthesis-priority subset |
...
gate. `peptides.search(super_elite_affinity=True)` applies the structural
...
(affinity) as the synthesis-ready subset.
...
stored in NANOMOLAR (nM); the affinity gate compares to 100 nM, not 1e-7 M.
