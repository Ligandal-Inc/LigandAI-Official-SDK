...
class Proteins(Resource):
    ...
    ``/api/glycosylation-studio/*``, ``/api/user-proteins/*``."""
    ...
    def check_glycosylation(
        ...
        return GlycosylationData.model_validate(
            self._transport.request("GET", f"/api/glycosylation-studio/sites/{gene}", params=params)
            or {"gene": gene}
        )
...
class AsyncProteins(AsyncResource):
    ...
    async def check_glycosylation(
        ...
        if site_type is not None:
            params["site_type"] = site_type
        return GlycosylationData.model_validate(
            await self._transport.request("GET", f"/api/glycosylation-studio/sites/{gene}", params=params)
            or {"gene": gene}
        )
